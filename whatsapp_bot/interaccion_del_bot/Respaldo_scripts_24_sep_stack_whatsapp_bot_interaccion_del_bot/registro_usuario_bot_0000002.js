...
async function iniciarRegistroUsuario(msg, session, sessionFile) {
  ...
  switch(step){
    ...
    case 'aceptar_terminos':
      ...
      await msg.reply(`📄 Tu certificado PETBIO ha sido generado:`, { file: pdfPath });

      // Limpieza de sesión al final
      try { fs.unlinkSync(sessionFile); } catch(e){}
      return;
      ...
      try {
        ...
        const pdfPath = await generarCertificadoPDF({ ...data, petbioID: result.petbioID }, CERT_DIR);

        await msg.reply(`✅ Registro exitoso! Tu ID PETBIO es: ${result.petbioID}`);
        await msg.reply(`📄 Tu certificado PETBIO ha sido generado:`, { file: pdfPath });
      } catch (err) {
        console.error(err);
        await msg.reply('❌ Error interno al registrar. Volviendo al inicio...');
      } finally {
        try { fs.unlinkSync(sessionFile); } catch(e){}
      }
      return;

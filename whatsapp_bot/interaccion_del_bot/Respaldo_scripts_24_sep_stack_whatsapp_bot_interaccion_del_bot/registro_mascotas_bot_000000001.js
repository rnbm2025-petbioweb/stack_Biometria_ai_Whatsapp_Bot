...
async function iniciarRegistroMascota(msg, session, sessionFile) {
    ...
    try {
        ...
        switch(step) {
            case 'nombre':
                data.nombre = texto; session.step = 'edad';
                await msg.reply('📌 Escribe la edad de la mascota:');
                break;
            case 'edad':
                data.edad = texto; session.step = 'raza';
                await msg.reply('📌 Escribe la raza de la mascota:');
                break;
            case 'raza':
                data.raza = sugerirRaza(texto)[0] || texto; session.step = 'fotos';
                await msg.reply('📸 Envía las fotos de la mascota: perfil, laterales y ráfaga de 5 fotos.');
                break;
            case 'fotos':
                const rutaPerfil = await guardarImagen(msg, data.nombre);
                const archivos = {
                    perfil: rutaPerfil,
                    latdr: '/ruta/latdr.jpg',
                    latiz: '/ruta/latiz.jpg',
                    hf0: '/ruta/hf0.jpg',
                    hf15: '/ruta/hf15.jpg',
                    hf30: '/ruta/hf30.jpg',
                    hfld15: '/ruta/hfld15.jpg',
                    hfli15: '/ruta/hfli15.jpg'
                };

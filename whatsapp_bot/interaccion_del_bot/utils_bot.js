...
// ==========================
const BASE_STORAGE = path.join(__dirname, '../WhatsApp_bot_storage');

// Crear directorio base si no existe
if (!fs.existsSync(BASE_STORAGE)) fs.mkdirSync(BASE_STORAGE, { recursive: true });

// ==========================
// Subdirectorios seguros
// ==========================
const UPLOADS_DIR = path.join(BASE_STORAGE, 'uploads/mascotas');
if (!fs.existsSync(UPLOADS_DIR)) fs.mkdirSync(UPLOADS_DIR, { recursive: true });

const DOCUMENTS_DIR = path.join(BASE_STORAGE, 'documentos_bot');
if (!fs.existsSync(DOCUMENTS_DIR)) fs.mkdirSync(DOCUMENTS_DIR, { recursive: true });

const CERTS_DIR = path.join(BASE_STORAGE, 'certificados');
...
// ==========================
// Justificar texto para WhatsApp
// ==========================
function justificarTexto(texto, ancho = 40) {
    return texto.split('\n').map(line =>
        line.length > ancho
            ? line.match(new RegExp(`.{1,${ancho}}`, 'g')).join('\n')
            : line
    ).join('\n');
}

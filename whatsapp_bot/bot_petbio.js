...
// =====================
// Configuración MySQL (uso de pool para eficiencia)
// =====================
...
async function guardarImagen(texto) {
  ...
  try {
    ...
    if (fs.existsSync(posibleRuta)) rutaCompleta = posibleRuta;
    else throw new Error('❌ URL o archivo inválido para la imagen');
  }
...
async function registrarMascota(data, archivos) {
  ...
  try {
    ...
    const values = [
      safe(data.id_usuario), safe(data.nombre), safe(data.apellidos), safe(data.raza),
      safe(data.edad), safe(numero_documento_petbio.slice(-10)), safe(data.relacion || 'Propietario'),
      ...
      safe(data.numero_documento_externo || numero_documento_petbio.slice(-10)), safe(numero_documento_petbio),
    ...
    const pdfPythonPath = await generarPDFPython({ ...data, id_usuario: data.id_usuario, numero_documento_petbio, ruta_img_perfil: archivos.perfil });
...
async function registrarSuscripcion(data) {
  ...
  try {
    ...
    `;
    const values = [
      safe(data.nombres), safe(data.apellidos), safe(data.documento),
      safe(data.telefono), safe(data.whatsapp ? 1 : 0), safe(data.evidencia_pago), fecha
    ];
    const [result] = await connection.execute(sql, values);

...
$numero_registro = limpiar($_POST['numero_registro'] ?? generarNumeroRegistro());
...
function generarNumeroRegistro() {
  return "RNBM-" . rand(100000, 999999);
}
// Datos POST
...
$numero_registro = generarNumeroRegistro(); // Debes tener esto definido antes
#fecha_registro = date('Y-m-d');            // También asegúrate de que esto exista
...
$sql = "INSERT INTO registro_mascotas (
    ...
    numero_registro, fecha_registro,
    ruta_img_hf0, ruta_img_hf15, ruta_img_hf30, ruta_img_hfld15,
...
// Mapear rutas desde el array de imágenes
$ruta_img_hf0     = $imagenes['ruta_img_hf0']['ruta'] ?? '';
$ruta_img_hf15    = $imagenes['ruta_img_hf15']['ruta'] ?? '';
$ruta_img_hf30    = $imagenes['ruta_img_hf30']['ruta'] ?? '';
$ruta_img_hfld15  = $imagenes['ruta_img_hfld15']['ruta'] ?? '';
$ruta_img_hfli15  = $imagenes['ruta_img_hfli15']['ruta'] ?? '';
$ruta_img_perfil  = $imagenes['ruta_img_perfil']['ruta'] ?? '';
$ruta_img_latdr   = $imagenes['ruta_img_latdr']['ruta'] ?? '';
$ruta_img_latiz   = $imagenes['ruta_img_latiz']['ruta'] ?? '';

// bind_param: 28 valores (2 ints, 26 strings)

$stmt->bind_param(
    "isssissssssssssssssssssssssss",
    $id_usuario, $nombre, $apellidos, $raza, $edad, $relacion, $con_documento,
    $tipo_documento, $descripcion_documento, $entidad_expedidora, $numero_documento,
    $barrio, $ciudad, $ciudad_y_barrio, $codigo_postal,
    $documento_pasaporte, $clase_mascota, $condicion_mascota, $numero_documento_entidad,
    $numero_registro, // fecha_registro se omite, porque usamos NOW()
    $ruta_img_hf0, $ruta_img_hf15, $ruta_img_hf30, $ruta_img_hfld15,
    $ruta_img_hfli15, $ruta_img_perfil, $ruta_img_latdr, $ruta_img_latiz
);
...
if ($stmt->execute()) {
    ...
    // Estilos
    $color_titulo = [39, 68, 93];
    $color_texto  = [73, 125, 116];
    $fondo        = [239, 233, 213];
    ...
    // --- Cédula Horizontal con QR ---
    require_once(__DIR__ . '/libs/phpqrcode/qrlib.php');

    $documento_visible = $documento_pasaporte ?? $numero_documento ?? 'N/A';
    ...
    if ($perfil_path && file_exists($perfil_path)) {
        $cedula->Image($perfil_path, 3, 16, 20, 20);
    } else {
    ...
    $cedula->SetX(25); $cedula->Cell(0, 4, "Tipo: $clase_mascota", 0, 1);
    $cedula->SetX(25); $cedula->Cell(0, 4, "Raza: $raza", 0, 1);
    $cedula->SetX(25); $cedula->Cell(0, 4, "Edad: $edad años", 0, 1);
    $cedula->SetX(25); $cedula->Cell(0, 4, "Ciudad: $ciudad", 0, 1);
    $cedula->SetX(25); $cedula->Cell(0, 4, "ID PETBIO: $documento_visible", 0, 1);
...
$datos_cedula = [
    ...
    'ciudad'            => $ciudad,
    'edad'              => $edad . " años",
    'responsable'       => $apellidos, // o nombre del usuario
    'id_petbio'         => $numero_registro,
    'ruta_img_perfil'   => "/var/www/html" . $imagenes['ruta_img_perfil']['ruta'], // convertir a absoluta
...
// Convertir a JSON y pasarlo como argumento seguro al script
$json = json_encode($datos_cedula, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);

$cmd = "/home/josornol341/petbio11-formularios/biometria_ai/myenv/bin/python3 "
...
$pdf_absoluto = "/var/www/html/petbio_storage/documentos/cedula_{$numero_registro}.pdf";
if (file_exists($pdf_absoluto)) {
  ...
  // Cerrar conexiones antes de redireccionar
    $stmt->close();
    $conn->close();

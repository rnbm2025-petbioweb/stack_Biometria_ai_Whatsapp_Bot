...
#require_once('conexion_global.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($usuario && password_verify($password, $usuario['password'])) {
    ...
    // Opcional: limpieza de sesión antigua del mockup
    unset($_SESSION['paso_mockup_actual']);
    unset($_SESSION['datos_mockup']);
    unset($_SESSION['datos_biometricos']);
    unset($_SESSION['tipo_registro']);

    // Registrar ingreso
    $ip = $_SERVER['REMOTE_ADDR'] ?? 'desconocido';
    $navegador = $_SERVER['HTTP_USER_AGENT'] ?? 'sin user-agent';
    $insertLog = $conn->prepare("INSERT INTO registro_ingresos (id_usuario, direccion_ip, navegador) VALUES (:id_usuario, :ip, :navegador)");
    $insertLog->execute([
